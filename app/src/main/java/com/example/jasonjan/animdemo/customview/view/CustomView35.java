 ...
 * owner: jasonjan
 */
public class CustomView35 extends AppCompatImageView {
    ...
    public CustomView35(Context context) {
        this(context,null);
        init();
    }

    public CustomView35(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public CustomView35(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(){
        ...
        //初始化数据
        currentMatrix = new Matrix();//当前矩阵
        savedMatrix = new Matrix();//已保存的矩阵
        start = new PointF();
        mid = new PointF();
        mode = MODE_NONE;

        //资源相关
        bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.rita7);
        bitmap = Bitmap.createScaledBitmap(bitmap, bitmap.getWidth()/2,bitmap.getHeight()/2, true);
        setImageBitmap(bitmap);//ImageView的成员方法，设置图片
    }
